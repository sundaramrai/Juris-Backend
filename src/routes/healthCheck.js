...
let systemInfoCache = null, systemInfoCacheTime = 0;
...
const getSystemInfo = () => ({
    cpu: { model: os.cpus()[0].model, cores: os.cpus().length, loadAvg: os.loadavg() },
    os: { platform: os.platform(), release: os.release(), hostname: os.hostname() },
    process: { pid: process.pid, versions: process.versions, env: process.env.NODE_ENV }
});

const getDatabaseStatus = () => {
    try { return dbManager.getConnectionStatus(); }
    catch (err) { return { status: 'error', message: err.message }; }
};

const getCompleteSystemInfo = async () => {
    const now = Date.now();
    if (systemInfoCache && (now - systemInfoCacheTime < CACHE_TTL)) return systemInfoCache;
    ...
    const systemInfo = {
        ...
        ...getSystemInfo(),
        aiService: { status: aiStatus ? 'up' : 'down', metrics: getServiceMetrics() },
...
router.get('/', (req, res) => res.status(200).json({ status: 'up', timestamp: new Date().toISOString() }));
...
router.get('/errors', (req, res) => {
    try {
        ...
        res.status(500).json({ status: 'error', message: 'Failed to retrieve error logs' });
    }
});
