...
async function performDeepSearch(query, category = 'general', options = {}) {
  try {
    ...
    const searchParams = {
      ...
      cx: cx,
      q: enhancedQuery,
      num: options.deepSearch ? SEARCH_CONFIG.deepSearch.maxResults : SEARCH_CONFIG.maxResults,
    ...
    const timeout = options.deepSearch ? SEARCH_CONFIG.deepSearch.timeout : SEARCH_CONFIG.timeout;
...
async function searchWithImages(query, category = 'general') {
  try {
    const [textResults, imageResults] = await Promise.all([
      ...
      performDeepSearch(query, category, { includeImages: true, deepSearch: false })
    ]);
  ...
  try {
    ...
    if (options.includeRelated && results && results.length > 0) {
      ...
      const relatedResults = await Promise.all(
        relatedQueries.slice(0, 2).map(q =>
          performDeepSearch(q, category, { ...searchOptions, deepSearch: false })
        )
      );
...
function extractDate(item) {
  const dateStr =
    ...
    item.snippet?.match(/\b(\d{1,2}\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{4})\b/i)?.[0];
...
function detectCategory(topic) {
  ...
  const categoryKeywords = {
    criminal: ['ipc', 'crpc', 'criminal', 'arrest', 'bail', 'fir', 'police', 'theft', 'murder', 'assault'],
    civil: ['cpc', 'civil', 'contract', 'suit', 'injunction', 'damages', 'decree'],
    constitutional: ['constitution', 'fundamental rights', 'writ', 'supreme court', 'article', 'pil'],
    tax: ['gst', 'income tax', 'tax', 'tds', 'assessment', 'revenue'],
    family: ['marriage', 'divorce', 'custody', 'maintenance', 'alimony', 'domestic violence'],
    property: ['property', 'land', 'rera', 'real estate', 'registration', 'mutation'],
...
function formatSearchResultsWithImages(textResults, imageResults, options = {}) {
  let formatted = '';

  if (textResults && textResults.length > 0) {
    formatted += formatSearchResults(textResults, { ...options, maxResults: 5 });
  }
...
module.exports = {
  searchLegalUpdates: performDeepSearch,
  performDeepSearch,
